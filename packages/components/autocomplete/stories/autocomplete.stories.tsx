...
import {autocomplete, input, button} from "@tw-material/theme";
import {
  Pokemon,
  usePokemonList,
  animalsData,
...
import {Avatar} from "@tw-material/avatar";
...
const DynamicTemplate = ({variant, ...args}: AutocompleteProps<Animal>) => (
  <Autocomplete
    className="max-w-xs"
...
const RequiredTemplate = ({variant, ...args}: AutocompleteProps) => {
  return (
    <form
...
const MirrorTemplate = ({variant, ...args}: AutocompleteProps) => (
  <div className="w-full max-w-xl flex flex-row gap-4">
    <Autocomplete className="max-w-xs" label="Select an animal" variant={variant} {...args}>
      {items}
    </Autocomplete>
    <Autocomplete
      className="max-w-xs"
...
const LabelPlacementTemplate = ({variant, ...args}: AutocompleteProps) => (
  <div className="w-full flex flex-col items-center gap-12">
    <div className="w-full max-w-2xl flex flex-col gap-3">
      <h3>Without placeholder</h3>
      <div className="w-full flex flex-row items-end gap-4">
        <Autocomplete label="Select an animal" variant={variant} {...args}>
          {items}
        </Autocomplete>
        <Autocomplete label="Select an animal" variant={variant} {...args} labelPlacement="outside">
          {items}
        </Autocomplete>
        <Autocomplete
...
const AsyncLoadingTemplate = ({variant, ...args}: AutocompleteProps<Pokemon>) => {
...
const StartContentTemplate = ({variant, ...args}: AutocompleteProps) => (
  <Autocomplete
    className="max-w-xs"
...
const EndContentTemplate = ({variant, ...args}: AutocompleteProps) => (
  <Autocomplete
    className="max-w-xs"
    defaultSelectedKey={"cat"}
...
const DynamicTemplateWithDescriptions = ({variant, ...args}: AutocompleteProps<Animal>) => (
  <Autocomplete
    className="max-w-xs"
    defaultItems={animalsData}
    label="Favorite Animal"
...
const ItemStartContentTemplate = ({variant, ...args}: AutocompleteProps<Animal>) => (
  <Autocomplete className="max-w-xs" label="Select country" variant={variant} {...args}>
    <AutocompleteItem
      key="argentina"
...
const ControlledTemplate = ({variant, ...args}: AutocompleteProps<Animal>) => {
...
const CustomItemsTemplate = ({variant, ...args}: AutocompleteProps<User>) => (
  <Autocomplete
...
const WithSectionsTemplate = ({variant, ...args}: AutocompleteProps<User>) => (
  <Autocomplete className="max-w-xs" label="Favorite Animal" variant={variant} {...args}>
...
const WithCustomSectionsStylesTemplate = ({variant, ...args}: AutocompleteProps<User>) => {
...
const WithAriaLabelTemplate = ({variant, ...args}: AutocompleteProps) => (
  <Autocomplete className="max-w-xs" label="Favorite Animal" variant={variant} {...args}>
    {items}
  </Autocomplete>
);

const CustomStylesTemplate = ({variant, ...args}: AutocompleteProps<User>) => {
  return (
    <Autocomplete
      className="max-w-xs"
      classNames={{
...
const CustomStylesWithCustomItemsTemplate = ({...args}: AutocompleteProps<User>) => {
  return (
    <Autocomplete
