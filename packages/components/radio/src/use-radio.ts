...
import type {RadioVariantProps, RadioSlots, SlotsToClasses} from "@tw-material/theme";
import {Ref, ReactNode, useCallback, useId, useState} from "react";
import {useMemo, useRef} from "react";
import {useFocusRing} from "@react-aria/focus";
import {useHover, usePress} from "@react-aria/interactions";
import {radio} from "@tw-material/theme";
...
interface Props extends Omit<HTMLProps<"input">, keyof RadioVariantProps> {
  /**
   * Ref to the DOM node.
   */
  ref?: Ref<HTMLElement>;
...
export function useRadio(props: UseRadioProps) {
  ...
  const slots = useMemo(
    ...
    [size, isDisabled, isInvalid, disableAnimation],
  );
  ...
  const getBaseProps: PropGetter = useCallback(
    (props = {}) => {
      return {
        ...
        "data-hover-selected": dataAttr(isHovered && isSelected),
