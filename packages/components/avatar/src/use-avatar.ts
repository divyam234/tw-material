import type {AvatarSlots, AvatarVariantProps, SlotsToClasses} from "@tw-material/theme";

import {avatar} from "@tw-material/theme";
import {HTMLTwM3Props as HTMLMProps, PropGetter} from "@tw-material/system";
import {mergeProps} from "@react-aria/utils";
import {useDOMRef} from "@tw-material/react-utils";
import {safeText, dataAttr} from "@tw-material/shared-utils";
...
interface Props extends HTMLMProps<"span"> {
  /**
   * Ref to the DOM node.
   */
...
export function useAvatar(props: UseAvatarProps = {}) {
  ...
  const slots = useMemo(
    ...
    [radius, size, isBordered, isDisabled, isInGroup, groupContext?.isGrid],
  );
